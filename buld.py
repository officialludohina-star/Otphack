...
# === Tumhara Target Repository Yahan Set Kar Diya Hai ===
TARGET_OWNER = "officialludohina-star"
TARGET_REPO = "Otphack"
...
def ensure_repo_exists(token):
    # Ab yeh seedha tumhara diya hua repo check karega
    ...
    # Agar repo nahi mila, toh check karo ke token us account ka hai ya nahi
    user_info = api_request("https://api.github.com/user", token)
    auth_user = user_info.get("login")
    
    if auth_user != TARGET_OWNER:
        print(f"\n[ERROR] Authenticated user ({auth_user}) cannot create repository under '{TARGET_OWNER}'.")
        print("Please make sure your GitHub token belongs to the correct account or has access to that organization.")
        sys.exit(1)

    print(f"-> Creating new permanent repository '{TARGET_REPO}'...")
    payload = {"name": TARGET_REPO, "private": True}
...
def main():
    ...
    # Repo check ab naye function se hoga
    ...
    # Remote URL ab tumhare specific repo ka hai
    remote_url = f"https://{username}:{token}@github.com/{TARGET_OWNER}/{TARGET_REPO}.git"
    run_command("git remote remove origin 2>/dev/null")
    run_command(f"git remote add origin {remote_url}")

    # API URL bhi specific repo ka
    runs_url = f"https://api.github.com/repos/{TARGET_OWNER}/{TARGET_REPO}/actions/runs?per_page=10"
    ...
    # Actions URL ab tumhare repo ka
    actions_url = f"https://github.com/{TARGET_OWNER}/{TARGET_REPO}/actions"
